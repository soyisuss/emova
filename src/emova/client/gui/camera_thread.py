            ...
            if self.pipeline:
                ...
                if len(results) > 0 and self.sampler.should_process():
                    ...
                    if len(self._tensor_batch_buffer) >= self.batch_size:
                        ...
                        batch_tensor = np.stack(self._tensor_batch_buffer, axis=0)
                        ...
                        print(f"\n[IA-CORE] Lote de {self.batch_size} tensores procesado -> Emoción dominante: {emotion} | Confianza media: {conf * 100:.1f}%", flush=True)
